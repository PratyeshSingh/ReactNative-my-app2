...
export default function ProfileScreen({ user: userProp }: ProfileScreenProps) {
  ...
  const { user, loading, error } = useUserProfile(userId, userProp);

  // If a user object is provided directly, render it immediately (useful for auth flow demos)
  if (userProp) {
    const mapped = { id: userProp.id, name: userProp.name ?? `${userProp.firstName ?? ''} ${userProp.lastName ?? ''}`, email: userProp.email, image: userProp.image };
